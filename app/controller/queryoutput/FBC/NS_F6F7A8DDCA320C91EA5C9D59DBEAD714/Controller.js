Ext.define('Form.controller.queryoutput.FBC.NS_F6F7A8DDCA320C91EA5C9D59DBEAD714.Controller', {
   	extend: 'Ext.app.Controller',
   	views: ['queryoutput.FBC.NS_F6F7A8DDCA320C91EA5C9D59DBEAD714.View'],
   	models: ['queryoutput.FBC.NS_F6F7A8DDCA320C91EA5C9D59DBEAD714.Model'],
   	stores: ['queryoutput.FBC.NS_F6F7A8DDCA320C91EA5C9D59DBEAD714.Store'],
   		init: function() {
   			this.control({
   				'panel': {
   					render: this.initPanel
   				},
   				'NS_F6F7A8DDCA320C91EA5C9D59DBEAD714 button': {
   					click: this.topBarButtonClicked
   				},
   				'NS_F6F7A8DDCA320C91EA5C9D59DBEAD714': {
			...
			topBarButtonClicked: function(b) {
	   		if (b.action) {
				...
				var dw = Ext.create('Ext.window.Window', {
				    ...
				    items: [{
				    	xtype: 'form',
				    	id: 'viewform25831614-92E0-A6D4-E0E5D577F234C9A5',
				    	autoScroll: true,
				    	items: [{
							width: '95%',name: 'eginpmsbehavioralcomp-BEHAVIORALCOMPCODE',
							fieldLabel: 'Behavioral Competencies Code',
							padding: 10,
						 	labelAlign: 'top',
						 	xtype: 'displayfield'
						},{
							...
							fieldLabel: 'Behavioral Competency',
							padding: 10,
						 	labelAlign: 'top',
						 	xtype: 'displayfield'
						},{
							width: '95%',name: 'eginpmsbehavioralcomp-PMSCODEFK',
				...
				var fwin = Ext.create('Ext.window.Window', {
				    ...
				    buttons: [{
				    	...
				    	handler: function(em) {
				    		...
				    		if (dform.isValid()) {
				    			var dgrid = Ext.widget('NS_F6F7A8DDCA320C91EA5C9D59DBEAD714');
				...
				if (dgdata) df.setValues(dgdata.data);
	       		...
	       		Ext.Msg.show({
					...
					var qrytitle = 'PMS Behavioral Competencies';
					var qrycode = 'F6F7A8DD-CA32-0C91-EA5C9D59DBEAD714';
					Ext.NS_F6F7A8DDCA320C91EA5C9D59DBEAD714.Data.exportGridToExcel(qrycode, qrytitle, function(res) {
						dgrid.getEl().unmask();
						if (res.result !== true) {
				    ...
				    buttons: [{
				    	...
				    	handler: function(em) {
				    		...
				    		if (dform.isValid()) {
					    		...
					    		Ext.NS_F6F7A8DDCA320C91EA5C9D59DBEAD714.Data.emailSelGridQuery(fromv,tov,subjectv,bodyv,function(res) {
					    			dwin.getEl().unmask();
					    			if (res.success !== true) {
   		...
   		gridRenderingComplete: function(dis, eopts) {
   			var qc = 'F6F7A8DD-CA32-0C91-EA5C9D59DBEAD714';
   			Ext.NS_F6F7A8DDCA320C91EA5C9D59DBEAD714.Data.getUserToolbars(qc, function(res) {
   				if (res.success) {
   					dis.addDocked(res);
   				}
   			});
   		...
   		gridRecordedSelected: function(dis, rec, ind) {
   			var formCmp = Ext.getCmp('viewform25831614-92E0-A6D4-E0E5D577F234C9A5') || Ext.getCmp('autoeformididid');
   			if (formCmp) {
